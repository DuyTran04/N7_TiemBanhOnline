...
<marquee behavior="scroll" direction="left" scrollamount="6">Welcome to TiemBanh!</marquee>
...
<section class="p-50">
    ...
    <div class="container">
                ...
                <tr>
                    <td colspan="3">
                        <form action="./?controller=cart&action=applyCoupon" method="POST" class="coupon">
                            <input value="<?= !empty($coupon_id) ? $coupon_id : ''  ?>" type="text" placeholder="Nhập mã phiếu giảm giá" id="coupon_id" name="coupon_id">

                            <button type="submit" class="site-btn">Enter</button>
                            <?php if (isset($coupon)) : ?>
                                <?php if ($coupon == 0) : ?>
                                    <div>
                                        <small class="invalid-error">Phiếu giảm giá không hợp lệ</small>
                                    </div>
                                <?php endif; ?>
                                <?php if ($coupon != 0) : ?>
                                    <div>
                                        <small style="color: #28a745">Phiếu giảm giá đã hợp lệ</small>
                                    </div>
                                <?php endif; ?>
                            <?php endif; ?>
                        </form>
                    ...
                    <td colspan="2">
                        <h4 id="total-in-cart">Tổng giá: $<?= number_format($cart->total_price, 2, '.', '') ?></h4>
                    </td>
                </tr>
        ...
        <h3 style="margin-bottom: 1.2rem;">Thông tin Order</h3>
        <table class="order-info-table" style="width: 100%;">
            ...
            <tfoot>
                <tr>
                    <td colspan="1">
                        <a href="./?controller=product&action=allProducts" class="cs-btn border-root btn-white">Quay lại</a>
                    </td>

                    <td class="text-right">
                        <a href="./?controller=checkout" class="btn-root ptc-btn border-root">Đặt hàng</a>
                    </td>
                </tr>
            </tfoot>
        </table>
